            ...
            <div class="card-body">
                <form action="{{ route('admin.landing-pages.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-12">
                            <label class="form-label fw-medium">Linked Products <span class="text-danger">*</span></label>
                            <p class="text-muted small mb-2">Select one or multiple products for this landing page. The first selected will be the primary product.</p>

                            {{-- Hidden inputs will be generated by JS --}}
                            <div id="product-ids-hidden-inputs"></div>

                            {{-- Searchable multi-select UI --}}
                            <div class="product-multiselect-wrapper border rounded p-2" style="background:#fff; min-height:46px;">
                                {{-- Selected pills --}}
                                <div id="selected-products-pills" class="d-flex flex-wrap gap-1 mb-1"></div>
                                {{-- Search input --}}
                                <input type="text" id="product-search-input" class="form-control form-control-sm border-0 shadow-none p-1" placeholder="Search and select products..." autocomplete="off" style="outline:none;">
                            </div>
                            {{-- Dropdown list --}}
                            <div id="product-dropdown-list" class="border rounded mt-1 d-none" style="max-height:220px; overflow-y:auto; background:#fff; z-index:1050; position:relative;">
                            </div>
                            ...
                            {{-- Pass products to JS --}}
                            <script>
                                window.__lpProducts = @json($products->map(fn($p) => ['id' => $p->id, 'name' => $p->name]));
                                window.__lpSelectedIds = @json(old('product_ids', []));
                            </script>
                        </div>
...
.hover-icon-btn:hover i { color: #fff !important; }
.hover-icon-btn:hover span { color: rgba(255, 255, 255, 0.8) !important; }

/* Product Multi-select */
.product-multiselect-wrapper { cursor: text; transition: border-color 0.2s; }
.product-multiselect-wrapper:focus-within {
    border-color: #0d6efd !important;
    box-shadow: 0 0 0 0.2rem rgba(13, 110, 253, 0.15);
}
.product-pill {
    display: inline-flex; align-items: center; gap: 4px;
    background: #e7f0ff; border: 1px solid #b3ccff; color: #0d47a1;
    border-radius: 20px; padding: 2px 10px; font-size: 0.82rem; font-weight: 500;
}
.product-pill.primary-pill { background: #d1e7dd; border-color: #a3cfbb; color: #0a3622; }
.product-pill .pill-remove { cursor: pointer; font-size: 1rem; line-height: 1; opacity: 0.6; }
.product-pill .pill-remove:hover { opacity: 1; color: #dc3545; }
.lp-dropdown-item {
...
.lp-dropdown-item:hover, .lp-dropdown-item.active { background: #f0f4ff; }
.lp-dropdown-item.selected-item { background: #e7f0ff; color: #0d47a1; }
.lp-dropdown-item .check-icon { color: #0d6efd; }
</style>
@endsection

@push('scripts')
<script>
    // ===== PRODUCT MULTI-SELECT WIDGET =====
    (function initProductMultiSelect() {
        const allProducts = window.__lpProducts || [];
        const preSelectedIds = (window.__lpSelectedIds || []).map(Number);
        let selectedIds = [...preSelectedIds];

        const pillsContainer = document.getElementById('selected-products-pills');
        const searchInput = document.getElementById('product-search-input');
        const dropdown = document.getElementById('product-dropdown-list');
        const hiddenContainer = document.getElementById('product-ids-hidden-inputs');

        if (!pillsContainer || !searchInput || !dropdown || !hiddenContainer) return;
        ...
        function renderPills() {
            ...
            selectedIds.forEach(function(id, index) {
                ...
                pill.className = 'product-pill' + (index === 0 ? ' primary-pill' : '');
                pill.innerHTML = (index === 0 ? '<i class="bi bi-star-fill me-1" style="font-size:0.7rem;"></i>' : '') +
                    '<span>' + prod.name + '</span>' +
                    '<span class="pill-remove ms-1" data-id="' + id + '">&times;</span>';
        ...
        function renderDropdown(query) {
            ...
            if (filtered.length === 0) {
                dropdown.innerHTML = '<div class="lp-dropdown-item text-muted">No products found.</div>';
            } else {
                filtered.forEach(function(prod) {
                    const isSelected = selectedIds.includes(prod.id);
                    const item = document.createElement('div');
                    item.className = 'lp-dropdown-item' + (isSelected ? ' selected-item' : '');
                    item.dataset.id = prod.id;
                    item.innerHTML = (isSelected ? '<i class="bi bi-check-circle-fill check-icon"></i>' : '<i class="bi bi-circle text-muted"></i>') +
                        '<span>' + prod.name + '</span>';
        ...
        document.querySelector('.product-multiselect-wrapper').addEventListener('click', function() {
            searchInput.focus();
        });
