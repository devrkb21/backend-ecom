                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Optional description for internal reference">{{ old('description', $coupon->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control @error('minimum_order_amount') is-invalid @enderror" 
                                       id="minimum_order_amount" name="minimum_order_amount" value="{{ old('minimum_order_amount', $coupon->minimum_order_amount) }}">
                            </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" min="1" class="form-control @error('usage_limit_per_user') is-invalid @enderror" 
                                   id="usage_limit_per_user" name="usage_limit_per_user" value="{{ old('usage_limit_per_user', $coupon->usage_limit_per_user) }}"
                                   placeholder="Unlimited">
                    ...
                    <div class="row g-3">
                            ...
                            <div class="border rounded p-2" style="max-height: 200px; overflow-y: auto; background-color: #fff;">
                                @foreach($categories as $category)
                                    <div class="form-check mb-1">
                                        <input class="form-check-input" type="checkbox" name="applicable_categories[]" value="{{ $category->id }}" id="app_cat_{{ $category->id }}" {{ in_array($category->id, old('applicable_categories', $coupon->applicable_categories ?? [])) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="border rounded p-2" style="max-height: 200px; overflow-y: auto; background-color: #fff;">
                                @foreach($products as $product)
                                    <div class="form-check mb-1">
                                        <input class="form-check-input" type="checkbox" name="applicable_products[]" value="{{ $product->id }}" id="app_prod_{{ $product->id }}" {{ in_array($product->id, old('applicable_products', $coupon->applicable_products ?? [])) ? 'checked' : '' }}>
                        ...
                        <div class="col-12">
                            ...
                            <div class="border rounded p-2" style="max-height: 200px; overflow-y: auto; background-color: #fff;">
                                @foreach($products as $product)
                                    <div class="form-check mb-1">
                                        <input class="form-check-input" type="checkbox" name="excluded_products[]" value="{{ $product->id }}" id="exc_prod_{{ $product->id }}" {{ in_array($product->id, old('excluded_products', $coupon->excluded_products ?? [])) ? 'checked' : '' }}>
        ...
        <div class="col-lg-4">
                    ...
                    <div class="row g-3 text-center">
                        <div class="col-6">
                            <div class="border rounded p-3">
                                <div class="fs-4 fw-bold text-primary">{{ $coupon->used_count }}</div>
                                <div class="text-muted small">Times Used</div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="border rounded p-3">
                                <div class="fs-4 fw-bold text-success">৳{{ number_format($coupon->usages->sum('discount_amount'), 2) }}</div>
                                <div class="text-muted small">Total Discounts</div>
                            </div>
                        </div>
                    </div>
                    <div class="mt-3">
                        <span class="badge bg-{{ ['active' => 'success', 'inactive' => 'secondary', 'expired' => 'danger', 'scheduled' => 'info', 'exhausted' => 'warning'][$coupon->status] ?? 'secondary' }} fs-6">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input type="checkbox" class="form-check-input" id="free_shipping" name="free_shipping" value="1" {{ old('free_shipping', $coupon->free_shipping) ? 'checked' : '' }}>
