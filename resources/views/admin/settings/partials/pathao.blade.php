@php
    ...
    $sentToCourierCount = $sentToCourierCountPathao ?? 0;
    $pendingSendCount = $pendingSendCountPathao ?? 0;
    $stores = $pathaoStores ?? [];
    $connectionSuccess = $pathaoConnectionSuccess ?? false;
    $connectionMessage = $pathaoConnectionMessage ?? '';
    $merchantInfo = $pathaoMerchantInfo ?? [];
@endphp
...
<ul class="nav nav-tabs mb-4 border-bottom-0" id="pathaoTabs" role="tablist">
    <li class="nav-item" role="presentation">
        <button class="nav-link active px-4 py-2 text-dark bg-transparent rounded-top border-bottom border-3 border-pathao-tab fw-semibold" id="pathao-dashboard-tab" data-bs-toggle="tab" data-bs-target="#pathao-dashboard" type="button" role="tab" aria-controls="pathao-dashboard" aria-selected="true">
            <i class="bi bi-speedometer2 me-2 text-pathao-brand"></i> Dashboard
        </button>
    </li>
    <li class="nav-item" role="presentation">
        <button class="nav-link px-4 py-2 text-muted bg-transparent border-0" id="pathao-settings-tab" data-bs-toggle="tab" data-bs-target="#pathao-settings" type="button" role="tab" aria-controls="pathao-settings" aria-selected="false">
...
<div class="tab-content" id="pathaoTabsContent">
    ...
    <!-- DASHBOARD TAB -->
    <div class="tab-pane fade show active" id="pathao-dashboard" role="tabpanel" aria-labelledby="pathao-dashboard-tab">
    ...
    <!-- SETTINGS TAB -->
    <div class="tab-pane fade" id="pathao-settings" role="tabpanel" aria-labelledby="pathao-settings-tab">
        <div class="row">
                ...
                <form action="{{ route('admin.settings.couriers.pathao.update') }}" method="POST">
                    ...
                    <div class="card border shadow-sm mb-4">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="text-muted {{ $isChecked('pathao_enabled') ? 'd-none' : '' }}" data-integration-note="pathao_enabled">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const pathaoTabs = document.querySelectorAll('#pathaoTabs .nav-link');
        pathaoTabs.forEach(tab => {
            tab.addEventListener('shown.bs.tab', function (event) {
                pathaoTabs.forEach(t => {
